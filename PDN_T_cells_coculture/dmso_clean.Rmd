...
# Figure : MHC-I Score Violin Plot
```{r fig2_mhci}
...
# Figure : T-cell Dose Response Score
...
# Figure : Gene Expression Heatmap
```{r fig4_heatmap}
# Get top 30 genes
...
# Figure: Combined Violin Plots (MHCI Score + T-cell Dose Response Score)
```{r fig_combined_violin}
library(patchwork)
dosage_colors <- c('NoT' = '#023047', '0.5T' = '#FFD166')
# Extract the data
score_data <- as.data.frame(colData(cds)[, c("Tcell_Treatment", "APM_MHC_CI_score", "Tcell_dose_response_score")])
# Reshape to long format
score_data_long <- score_data %>%
  pivot_longer(cols = c("APM_MHC_CI_score", "Tcell_dose_response_score"),
names_to = "Score_Type",
values_to = "Score") %>%
  mutate(Score_Type = factor(Score_Type, 
levels = c("APM_MHC_CI_score", "Tcell_dose_response_score"),
labels = c("MHCI", "TCDG")))
# Create the plot with facets
p <- ggplot(score_data_long, 
       aes(x = factor(Tcell_Treatment, levels = c('NoT', '0.5T')), 
y = Score, 
fill = factor(Tcell_Treatment, levels = c('NoT', '0.5T')))) +
  geom_violin(color = "black", size = 0.3) +
  stat_summary(fun = mean, geom = "point", size = 0.5) +
  scale_fill_manual(values = dosage_colors) +
  facet_wrap(~Score_Type, ncol = 1, scales = "free_y", strip.position = "top") +
  labs(x = "T cell:GBM    ", 
y = "Expression") +
  theme_classic() +
...
p
ggsave(paste0(out_dir, "/combined_violin_plots.png"), width = 1, height = 2, dpi = 600)
ggsave(paste0(out_dir, "/combined_violin_plots.pdf"), width = 1, height = 2, dpi = 600)
ggsave(paste0(out_dir, "/combined_violin_plots.svg"), width = 1, height = 2, dpi = 600)
