...
base_dir <- paste0("/burg/cgl/users/ls3456_v2/GBM_Tcells_CRISPRA/", pert_type, "_final/")
setwd(base_dir)
...
for(kinase in kinase_perturbation){
  ...
  for(gene_set in c("APM_MHC_CI_score","APM_MHC_CII_score","Tcell_response","IFNG","NFKB")){
    ...
    score_df_subset <- colData(cds) %>% as.data.frame() %>% filter(gene_id %in% c("NTC",kinase), dose == 0)
