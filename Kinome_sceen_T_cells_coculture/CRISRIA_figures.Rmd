...
```{r}
# keep only the either qvalue columns < 0.01 
...
```{r}
# Create a column with the minimum p-value from either q_value_a or q_value_i
# Then select top 30 genes per signature
gene_set_combined_top30_by_sig <- gene_set_combined %>%
  filter((!is.na(q_value_a) & q_value_a < 0.05) | 
         (!is.na(q_value_i) & q_value_i < 0.05)) %>%
  mutate(q_value_min = pmin(q_value_a, q_value_i, na.rm = TRUE)) %>%
  group_by(signature) %>%
  slice_min(q_value_min, n = 30) %>%
  ungroup()

# View the results
head(gene_set_combined_top30_by_sig)

# Check how many genes per signature
gene_set_combined_top30_by_sig %>% count(signature)

# Save to CSV
write.csv(gene_set_combined_top30_by_sig, "/home/user/Documents/Kinase_project/figures/crispria_combined/figures_degs/gene_set_combined_top30_by_signature.csv", row.names = FALSE)
...
```{r}
gene_set_combined_top30_by_sig$q_value_a <- NULL
gene_set_combined_top30_by_sig$q_value_i <- NULL
head(gene_set_combined_top30_by_sig)
```

```{r}
split_data_heatmap <- split(gene_set_combined_top30_by_sig, gene_set_combined_top30_by_sig$signature)
# Access the subsets by their names
APM_MHC_CI_score_data_matrix_top30_by_sig <- split_data_heatmap$APM_MHC_CI_score
APM_MHC_CII_score_data_matrix_top30_by_sig <- split_data_heatmap$APM_MHC_CII_score
Tcell_response_data_matrix_top30_by_sig <- split_data_heatmap$Tcell_response
INFG_data_matrix_top30_by_sig <- split_data_heatmap$IFNG
NFKB_data_matrix_top30_by_sig <- split_data_heatmap$NFKB
head(Tcell_response_data_matrix_top30_by_sig)
```
...
top_APM_MHC_CI_genes_matrix_top30_by_sig <- APM_MHC_CI_score_data_matrix_top30_by_sig %>% 
  select(gene_id, beta_i, beta_a) %>% 
  rename(CRISPRi = beta_i, CRISPRa = beta_a) %>%
  as.data.frame()

row.names(top_APM_MHC_CI_genes_matrix_top30_by_sig) <- top_APM_MHC_CI_genes_matrix_top30_by_sig$gene_id
top_APM_MHC_CI_genes_matrix_top30_by_sig$gene_id <- NULL
hmcols <- colorRampPalette(c("blue","white","red"))(35)

paletteLength <- 35
myBreaks <- c(seq(min(top_APM_MHC_CI_genes_matrix_top30_by_sig), 0, length.out=ceiling(paletteLength/2) + 1),
              seq(max(top_APM_MHC_CI_genes_matrix_top30_by_sig)/paletteLength, max(top_APM_MHC_CI_genes_matrix_top30_by_sig), length.out=floor(paletteLength/2)))

annotation_df <- data.frame(row.names = colnames(top_APM_MHC_CI_genes_matrix_top30_by_sig), 
                            condition_id = colnames(top_APM_MHC_CI_genes_matrix_top30_by_sig))


```

```{r}
pheatmap::pheatmap(top_APM_MHC_CI_genes_matrix_top30_by_sig,
...
# only keep beta column
top_APM_MHC_CII_genes_matrix_top30_by_sig <- APM_MHC_CII_score_data_matrix_top30_by_sig %>% 
  select(gene_id, beta_i,beta_a) %>% 
  rename(CRISPRi = beta_i, CRISPRa = beta_a) %>%
  as.data.frame()

row.names(top_APM_MHC_CII_genes_matrix_top30_by_sig) <- top_APM_MHC_CII_genes_matrix_top30_by_sig$gene_id
top_APM_MHC_CII_genes_matrix_top30_by_sig$gene_id <- NULL
hmcols <- colorRampPalette(c("blue","white","red"))(35)

paletteLength <- 35
myBreaks <- c(seq(min(top_APM_MHC_CII_genes_matrix_top30_by_sig), 0, length.out=ceiling(paletteLength/2) + 1),
              seq(max(top_APM_MHC_CII_genes_matrix_top30_by_sig)/paletteLength, max(top_APM_MHC_CII_genes_matrix_top30_by_sig), length.out=floor(paletteLength/2)))

annotation_df <- data.frame(row.names = colnames(top_APM_MHC_CII_genes_matrix_top30_by_sig), 
                            condition_id = colnames(top_APM_MHC_CII_genes_matrix_top30_by_sig))


```
```{r}
pheatmap::pheatmap(top_APM_MHC_CII_genes_matrix_top30_by_sig,
                   clustering_method = "ward.D2",
                   cluster_cols = FALSE,
                   color = hmcols,
                   breaks = myBreaks,
...
# only keep beta column
Tcell_response_genes_matrix_top30_by_sig <- Tcell_response_data_matrix_top30_by_sig %>% 
  select(gene_id, beta_i,beta_a) %>% 
  rename(CRISPRi = beta_i, CRISPRa = beta_a) %>%
  as.data.frame()

row.names(Tcell_response_genes_matrix_top30_by_sig) <- Tcell_response_genes_matrix_top30_by_sig$gene_id
Tcell_response_genes_matrix_top30_by_sig$gene_id <- NULL
hmcols <- colorRampPalette(c("blue","white","red"))(35)

paletteLength <- 35
myBreaks <- c(seq(min(Tcell_response_genes_matrix_top30_by_sig), 0, length.out=ceiling(paletteLength/2) + 1),
              seq(max(Tcell_response_genes_matrix_top30_by_sig)/paletteLength, max(Tcell_response_genes_matrix_top30_by_sig), length.out=floor(paletteLength/2)))

annotation_df <- data.frame(row.names = colnames(Tcell_response_genes_matrix_top30_by_sig), 
                            condition_id = colnames(Tcell_response_genes_matrix_top30_by_sig))


```
```{r}
pheatmap::pheatmap(Tcell_response_genes_matrix_top30_by_sig,
                   clustering_method = "ward.D2",
                   cluster_cols = FALSE,
                   color = hmcols,
                   breaks = myBreaks,
...
# only keep beta column
top_INFG_genes_matrix_top30_by_sig <- INFG_data_matrix_top30_by_sig %>% 
  select(gene_id, beta_i,beta_a) %>% 
  rename(CRISPRi = beta_i, CRISPRa = beta_a) %>%  
  as.data.frame()

row.names(top_INFG_genes_matrix_top30_by_sig) <- top_INFG_genes_matrix_top30_by_sig$gene_id
top_INFG_genes_matrix_top30_by_sig$gene_id <- NULL
hmcols <- colorRampPalette(c("blue","white","red"))(35)

paletteLength <- 35
myBreaks <- c(seq(min(top_INFG_genes_matrix_top30_by_sig), 0, length.out=ceiling(paletteLength/2) + 1),
              seq(max(top_INFG_genes_matrix_top30_by_sig)/paletteLength, max(top_INFG_genes_matrix_top30_by_sig), length.out=floor(paletteLength/2)))

annotation_df <- data.frame(row.names = colnames(top_INFG_genes_matrix_top30_by_sig), 
                            condition_id = colnames(top_INFG_genes_matrix_top30_by_sig))


```
```{r}
pheatmap::pheatmap(top_INFG_genes_matrix_top30_by_sig,
                   clustering_method = "ward.D2",
                   cluster_cols = FALSE,
                   color = hmcols,
                   breaks = myBreaks,
...
# only keep beta column
top_NFKB_genes_matrix_top30_by_sig <- NFKB_data_matrix_top30_by_sig %>% 
  select(gene_id, beta_i,beta_a) %>% 
  rename(CRISPRi = beta_i, CRISPRa = beta_a) %>% 
  as.data.frame()

row.names(top_NFKB_genes_matrix_top30_by_sig) <- top_NFKB_genes_matrix_top30_by_sig$gene_id
top_NFKB_genes_matrix_top30_by_sig$gene_id <- NULL
hmcols <- colorRampPalette(c("blue","white","red"))(35)

paletteLength <- 35
myBreaks <- c(seq(min(top_NFKB_genes_matrix_top30_by_sig), 0, length.out=ceiling(paletteLength/2) + 1),
              seq(max(top_NFKB_genes_matrix_top30_by_sig)/paletteLength, max(top_NFKB_genes_matrix_top30_by_sig), length.out=floor(paletteLength/2)))

annotation_df <- data.frame(row.names = colnames(top_NFKB_genes_matrix_top30_by_sig), 
                            condition_id = colnames(top_NFKB_genes_matrix_top30_by_sig))


```
```{r}
pheatmap::pheatmap(top_NFKB_genes_matrix_top30_by_sig,
                   clustering_method = "ward.D2",
                   cluster_cols = FALSE,
                   color = hmcols,
                   breaks = myBreaks,
...
```{r}
# Your four gene sets
genes1 <- APM_MHC_CI_score_data_matrix$gene_id
genes2 <- APM_MHC_CII_score_data_matrix$gene_id
genes3 <- INFG_data_matrix$gene_id
genes4 <- NFKB_data_matrix$gene_id
